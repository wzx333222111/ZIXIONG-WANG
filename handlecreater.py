...
def getdata(name):
    ...
    collection = htmll.xpath('//*[@id="main"]/div/div/div[3]/div/div[3]/div[2]/div/p/text()')
    collectionrt = 1
    # print(items)
    if len(collection) >0:
        collectionrt = 0
...
if __name__ == '__main__':
    ...
    for i in range(0 , 15000):
        try:
            print("______"+ str(i))
            arr = getdata('./info2html/createrAddrcreated' + str(i) +'.html')
            allarr.append(arr)
    ...
    name = ['collection' ,'creater' , 'fav' ,"activity" ]
    test = pandas.DataFrame(columns=name, data=allarr)
    test.to_csv("./csv/" + filename + 'createrAddrcreated.csv', encoding='utf-8')
