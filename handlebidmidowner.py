...
if __name__ == '__main__':
    ...
    for i in range(0 , 15000):
        try:
            print("______"+ str(i))
            arr = getdata('./info2html/ownerAddrRtbids_made' + str(i) +'.html')
            allarr.append(arr)
    ...
    name = ['collection' ,'creater' , 'fav' ,"ownerAddrRtbids_made" ]
    test = pandas.DataFrame(columns=name, data=allarr)
    test.to_csv("./csv/" + filename + 'ownerAddrRtbids_made.csv', encoding='utf-8')
