...
if __name__ == '__main__':
    ...
    for i in range(0 , 13000):
        try:
            print("______"+ str(i))
            arr = getdata('./infohtml/'+ filename + str(i) +'.html')
    ...
    name = [    'name' ,'collection addrRt' , 'collection' ,"ownerAddrRt" ,"ownerRz" ,"viewrt" ,"fav" ,"createrAddr" ,"createrrt" ,"alltiomPricrt " ,"是否有about", "是否有properties" ,"是否有levels", "是否有stats", "isList", "listPric" ,"isoffer" ,"isofferP", "salepri"]
    test = pandas.DataFrame(columns=name, data=allarr)
    test.to_csv("./csv/" + filename + 'v0505.csv', encoding='utf-8')
