...
if __name__ == '__main__':
    ...
    for i in range(0 , 15000):
        try:
            print("______"+ str(i))
            arr = getdata('./info2html/createrAddrbids_made' + str(i) +'.html')
            allarr.append(arr)
    ...
    name = ['collection' ,'creater' , 'fav' ,"bidmid" ]
    test = pandas.DataFrame(columns=name, data=allarr)
    test.to_csv("./csv/" + filename + 'createrAddrbidsmade.csv', encoding='utf-8')
